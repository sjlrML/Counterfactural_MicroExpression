...
def get_args():
    ...
    # parse.add_argument('--data_root',type=str,default='/data/laolingjie/CASME2/SeetaFace_Crop')
    parse.add_argument('--data_root',type=str,default='/data/laolingjie/database/MMEW/Micro_Expression')
    parse.add_argument('--data_list',type=str,default='/data/laolingjie/database/MMEW/MMEW_Micro_Exp_my.xlsx')
    ...
    parse.add_argument('--train_log',type=str,default='log/MMEW_baseline.log')
    parse.add_argument('--save_name',type=str,default='MMEW_baseline')
    parse.add_argument('--start_epoch',type=int,default=0)
    parse.add_argument('--nb_epoch',type=int,default=120)
    parse.add_argument('--batch_size',type=int,default=12)
    parse.add_argument('--num_workers',type=int,default=8)
    parse.add_argument('--classes_num',type=int,default=6)
    ...
    parse.add_argument('--beta',type=float,default=1,help='the loss weight of LDS loss')
...
def train(train_loader,model,CELoss,optimizer,epoch,args):
    ...
    for batch_idx,(image_list,labels,image_index_list,apex_index) in enumerate(train_loader): 
        ...
        if batch_idx % 5 == 0:
            print("Sample: {}/5 Iteration {} Step: {} | Loss: {:.3f} | Acc: {:.3f} ({}/{}) |" \
                 "Counterfactual Loss: {:.3f} | CE Loss: {:.3f} |".format(
                i_fold+1,epoch,batch_idx,train_loss / (batch_idx + 1),100. *float(correct)/total,correct,total,
                args.alpha *con_loss / (batch_idx+1),ce_loss / (batch_idx+1)))
            with open(args.train_log,'a') as file:
                file.write("Sample: {}/5 Iteration {} Step: {} | Loss: {:.3f} | Acc: {:.3f} ({}/{}) |" \
    ...
    print(
        'Iteration %d Sample: %d/5 | Acc = %.5f | F1_socre = %.5f | train_loss = %.5f |' % (
            epoch,i_fold+1,train_acc,f1,train_loss/ (batch_idx + 1)))
    with open(args.train_log,'a') as file:
        file.write(
            'Iteration %d Sample: %d/5 | Acc = %.5f | F1_socre = %.5f | train_loss = %.5f |\n' % (
            epoch,i_fold+1,train_acc,f1,train_loss/ (batch_idx + 1)))
...
if __name__ == "__main__":
    ...
    subject_index = MMEW_config["subject_num"]
    random.shuffle(subject_index)
    splits = split_list_n_list(subject_index,5)
    best_acc = []
    best_f1 = []
    best_correct = 0
    best_total = 0
    best_label = []
    best_pred = []
    for i_fold,test_subject_index in enumerate(splits):
        print("*****fold {}/5*****".format(i_fold+1))
        # pdb.set_trace()
        # print(test_subject_index)
        train_dataset = MMEW(args.data_root,args.data_list, class_num = args.classes_num,test_subject=test_subject_index,is_dynamic=False,
        ...
        val_dataset = MMEW(args.data_root,args.data_list,class_num = args.classes_num,test_subject=test_subject_index,is_dynamic=False,
        ...
        optimizer = optim.SGD([
            {'params': model.module.feature_extractor.parameters(),'lr':0.0005},
            {'params': model.module.classifier.parameters(),'lr':0.005},
            # {'params': model.module.lstm.parameters(),'lr':0.002},
        ],
        ...
        # lr = [0.00005,0.0005,0.0005,0.0005,0.0005]
        ...
        for epoch in range(args.start_epoch+1,args.nb_epoch+1):
            ...
            print('{} Sample: {}/5 Iteration {} finish!!!'.format(time_str,i_fold+1,epoch))
            with open(args.train_log,'a') as file:
                file.write('{} Sample: {}/5 Iteration {} finish!!!\n'.format(time_str,i_fold+1,epoch))
            if acc > 99.0:
                break
        ...
        print(
            'Sample {}/{} | Avg Acc = {:.5f} ({})/({}) | Avg F1_socre = {:.5f} |'.format(i_fold+1,len(subject_index),int(best_correct)/int(best_total)*100,best_correct,best_total,socre))
        with open(args.train_log,'a') as file:
            file.write(
                'Sample {}/{} | Avg Acc = {:.5f} | Avg F1_socre = {:.5f} |\n'.format(i_fold+1,len(subject_index),int(best_correct)/int(best_total)*100,best_correct,best_total,socre))
