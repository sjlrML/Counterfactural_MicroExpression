...
from model_pca_gcn import PCA_GCN_model
from dataset_config import CASME2_config
import pickle
import random
import pdb
...
def get_args():
    ...
    parse.add_argument('--data_root',type=str,default='/data/laolingjie/database/CASME2/Cropped')
    parse.add_argument('--data_list',type=str,default='/data/laolingjie/database/CASME2/CASME2-coding-20140508_2.xlsx')
    ...
    parse.add_argument('--train_log',type=str,default='log/CASME2_baseline.log')
    parse.add_argument('--save_name',type=str,default='CASME2_baseline')
...
#             output = model(image_list,phase="test")
...
#     return val_acc,f1,list(total_label),list(total_pred)
...
def train(train_loader,model,CELoss,optimizer,epoch,args):
    ...
    f1 = f1_score(fold_label,fold_pred,average='weighted')*100
...
if __name__ == "__main__":
    ...
    subject_index = CASME2_config["subject_num"]
    # subject_index = [i for i in range(1,3)]
    ...
    for i_fold,test_subject_index in enumerate(subject_index):
        ...
        train_dataset = CASME2(args.data_root,args.data_list, class_num = args.classes_num,test_subject=test_subject_index,is_dynamic=False,
        ...
        val_dataset = CASME2(args.data_root,args.data_list,class_num = args.classes_num,test_subject=test_subject_index,is_dynamic=False,
        ...
        # model = Base_model(backbone,768,args.classes_num)
        # model = Gaussian_model(backbone,256,args.classes_num,LSTM_layer=args.lstm_num)
        # model = GCN_model(backbone,256,args.classes_num)
        # model = PCA_GCN_model(backbone,512,args.classes_num)
        model = PCA_model(backbone,512,args.classes_num)
        ...
        optimizer = optim.SGD([
            {'params': model.module.feature_extractor.parameters(),'lr':0.0002},
            {'params': model.module.classifier.parameters(),'lr':0.002},
        ...
        best_pred.append(epoch_pred)
        best_label.append(epoch_label)
        socre = f1_score(best_label,best_pred,average='weighted')*100
        print(
            'Sample {}/{} | Avg Acc = {:.5f} | Avg F1_socre = {:.5f} |'.format(i_fold+1,len(subject_index),best_correct/best_total*100,socre))
        with open(args.train_log,'a') as file:
            file.write(
                'Sample {}/{} | Avg Acc = {:.5f} | Avg F1_socre = {:.5f} |\n'.format(i_fold+1,len(subject_index),best_correct/best_total*100,socre))
